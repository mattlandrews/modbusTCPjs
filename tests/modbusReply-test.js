...
describe("modbusReply", function () {
    ...
    it("#modbusReply() creates a valid readHoldingRegister reply (1 - [0])", function () {
        ...
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 3, transactionID: 0, byteCount: 2, exception: null });
    ...
    it("#modbusReply() creates a valid readHoldingRegister reply (2 - [1,2])", function () {
        ...
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 3, transactionID: 1, byteCount: 4, exception: null });
    ...
    it("#modbusReply() creates a valid writeHoldingRegisters reply (1)", function () {
        ...
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 16, transactionID: 2, exception: null });
    ...
    it("#modbusReply() creates a valid writeHoldingRegisters reply (2)", function () {
        ...
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 16, transactionID: 3, exception: null });
    ...
    it("#modbusReply() creates a valid exception - illegal function", function () {
        reply = new modbusReply([0x00, 0x04, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x01]);
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 128, transactionID: 4, exception: 1 });
        expect(reply.data).deep.equals([]);
        expect(reply.replyBuffer).deep.equals([0x00, 0x04, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x01]);
    });
    it("#modbusReply() creates a valid exception - illegal data address", function () {
        reply = new modbusReply([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x02]);
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 128, transactionID: 5, exception: 2 });
        expect(reply.data).deep.equals([]);
        expect(reply.replyBuffer).deep.equals([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x02]);
    });
    it("#modbusReply() creates a valid exception - illegal data value", function () {
        reply = new modbusReply([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x03]);
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 128, transactionID: 5, exception: 3 });
        expect(reply.data).deep.equals([]);
        expect(reply.replyBuffer).deep.equals([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x03]);
    });

    it("#modbusReply() creates a valid exception - slave device failure", function () {
        reply = new modbusReply([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x04]);
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 128, transactionID: 5, exception: 4 });
        expect(reply.data).deep.equals([]);
        expect(reply.replyBuffer).deep.equals([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x04]);
    });
    it("#modbusReply() creates a valid exception - acknowledge", function () {
        reply = new modbusReply([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x05]);
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 128, transactionID: 5, exception: 5 });
        expect(reply.data).deep.equals([]);
        expect(reply.replyBuffer).deep.equals([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x05]);
    });
    it("#modbusReply() creates a valid exception - slave device busy", function () {
        reply = new modbusReply([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x06]);
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 128, transactionID: 5, exception: 6 });
        expect(reply.data).deep.equals([]);
        expect(reply.replyBuffer).deep.equals([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x06]);
    });
    it("#modbusReply() creates a valid exception - negative acknowledge", function () {
        reply = new modbusReply([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x07]);
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 128, transactionID: 5, exception: 7 });
        expect(reply.data).deep.equals([]);
        expect(reply.replyBuffer).deep.equals([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x07]);
    });
    it("#modbusReply() creates a valid exception - memory parity error", function () {
        reply = new modbusReply([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x08]);
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 128, transactionID: 5, exception: 8 });
        expect(reply.data).deep.equals([]);
        expect(reply.replyBuffer).deep.equals([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x08]);
    });
    it("#modbusReply() creates a valid exception - gatway path unavailable", function () {
        reply = new modbusReply([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x0A]);
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 128, transactionID: 5, exception: 10 });
        expect(reply.data).deep.equals([]);
        expect(reply.replyBuffer).deep.equals([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x0A]);
    });
    it("#modbusReply() creates a valid exception - gateway target device failed to respond", function () {
        reply = new modbusReply([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x0B]);
        expect(reply).to.be.a("object")
            .and.contains({ id: 1, func: 128, transactionID: 5, exception: 11 });
        expect(reply.data).deep.equals([]);
        expect(reply.replyBuffer).deep.equals([0x00, 0x05, 0x00, 0x00, 0x00, 0x05, 0x01, 0x80, 0x0B]);
    });
